...
/**
 * Draw stick characters from skeleton data on top of depth image. (Adapted from
 * NiTE examples).
 */
public class BonesRenderer extends Component implements BonesListener {
    ...
    public synchronized void paint(Graphics g) {
        ...
        if (depthFrame != null) {
            ...
            this.xscale = g2.getClipBounds().width / (double) width;
            this.yscale = g2.getClipBounds().height / (double) height;
    ...
    private void drawLimb(Graphics g, int x, int y, UserData user, JointType from, JointType to) {
        ...
        g2.drawLine((int) (xscale * fromPos.getX().intValue()),
                (int) (yscale * fromPos.getY().intValue()),
                (int) (xscale * toPos.getX().intValue()),
                (int) (yscale * toPos.getY().intValue()));
    }
